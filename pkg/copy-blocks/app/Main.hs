...
{-
    This is a simple protocol for copying IPFS blocks from one node to another.

    What should this do?

    I must just need to shove all the data down the pipe, right? Even advertising which hashes I'm sending
      down the pipe is technically redundant.

    Trivial protocol is just:
        size(3B) block(nB) size(3B) block(nB) ...

    Trivial implementation is just:
        Server:
            Listen on a socket. For each new a connection:
                Read blocks until the connection closes.
                Queue the block for insertion.
            For each queued block:
                Insert it into the blockstore with `ipfs block put`
        Client:
            Open a connection to the server.
            Accept a stream of hashes, for each hash:
                Read the associated block with `ipfs block get`.
                Get the size of the block.
                Send the size (3 bytes).
                Send all the bytes from the block.
                If something failed during transfer:
                    Log it, debounce, reconnect, retransfer, continue

    What's the general shape of an implementation?
        Server:
            Socket stuff.
            One thread per each connection.
            Queue+worker thread for inserting blocks.
        Client:
            Thread to read lines from stdin and put them in a queue.
            Thread that gets hashes from a queue and dumps blocks in another queue.
            Thread that maintains a socket, reads blocks from a queue, and dumps them into the socket.

    What are the primitives that I need?
        Read/Write blocks from IPFS blockstore.
        Open/Close sockets.
        STM Queues
-}
